...
Why does styled-components rock? 

Well, as we learned today, not all CSS features work with JavaScript event handlers. For example,  pseudo selectors ( :disabled, :before, :nth-child) and you cannot style in the html and body tags either.

Darn! Soooo JS? 

YES!

With JS,  CSS is generated, you can use every media query and pseudo selector. Some libraries (like jss, styled-components) even add support for cooool, non-CSS-native features for example nesting.

Gone are the days of long style sheets. Here is CSS in JS. 

As we learned in class, some highlights of style-components 

**Automatic critical CSS:** styled-components keeps track of which components are rendered on a page and injects their styles and nothing else, fully automatically. Combined with code splitting, this means you load the least amount of code necessary.
